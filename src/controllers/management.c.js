...
module.exports = {
  ...
  getTeamManagement: async function (req, res) {
    ...
    const countActiveTournament = await TournamentModel.countActive();
    res.render('management/teams', {
      ...
      subNavigation: 0,
      teams: ownTeams || [],
      hasActiveTournament: Number(countActiveTournament) > 0,
    });
  },
  ...
  putAccountRole: async function (req, res, next) {
    ...
    if (!user.isAdmin && role !== 'team_manager') {
      return res.status(403).json({ status: 'error', message: 'Bạn không có quyền đặt vai trò này.' });
    }
    ...
    try {
      ...
      if (!user.isAdmin && targetUser.id === user.id && role === 'team_manager') {
        return res.status(400).json({ status: 'error', message: 'Không thể tự hạ quyền tài khoản hiện tại.' });
      }

      const updatedUser = await UserModel.updateUserRole(targetUser.id, role);
      if (!updatedUser) {
